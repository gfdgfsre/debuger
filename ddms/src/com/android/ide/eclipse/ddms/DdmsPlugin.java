 ...
 */
public final class DdmsPlugin extends AbstractUIPlugin
		implements IDeviceChangeListener, IUiSelectionListener,
		com.android.ddmuilib.StackTracePanel.ISourceRevealer
{
	...
	public void start(BundleContext context) throws Exception
	{
		...
		eclipseStore.addPropertyChangeListener(new IPropertyChangeListener()
		{
			...
			public void propertyChange(PropertyChangeEvent event)
			{
				...
				{
					DdmPreferences.setDebugPortBase(eclipseStore.getInt(
							PreferenceInitializer.ATTR_DEBUG_PORT_BASE));
				} else if (PreferenceInitializer.ATTR_SELECTED_DEBUG_PORT
						.equals(property))
				{
					DdmPreferences.setSelectedDebugPort(eclipseStore.getInt(
							PreferenceInitializer.ATTR_SELECTED_DEBUG_PORT));
				} else if (PreferenceInitializer.ATTR_THREAD_INTERVAL
						.equals(property))
				{
					DdmUiPreferences.setThreadRefreshInterval(eclipseStore
							.getInt(PreferenceInitializer.ATTR_THREAD_INTERVAL));
				} else if (PreferenceInitializer.ATTR_LOG_LEVEL
						.equals(property))
				{
					DdmPreferences.setLogLevel(eclipseStore
							.getString(PreferenceInitializer.ATTR_LOG_LEVEL));
				} else if (PreferenceInitializer.ATTR_TIME_OUT.equals(property))
				{
					DdmPreferences.setTimeOut(eclipseStore
							.getInt(PreferenceInitializer.ATTR_TIME_OUT));
				} else if (PreferenceInitializer.ATTR_USE_ADBHOST
						.equals(property))
				{
					DdmPreferences.setUseAdbHost(eclipseStore.getBoolean(
							PreferenceInitializer.ATTR_USE_ADBHOST));
				} else if (PreferenceInitializer.ATTR_ADBHOST_VALUE
						.equals(property))
				{
					DdmPreferences.setAdbHostValue(eclipseStore.getString(
							PreferenceInitializer.ATTR_ADBHOST_VALUE));
				}
			}
		});
		...
		new Job(Messages.DdmsPlugin_DDMS_Post_Create_Init)
		{
			...
			protected IStatus run(IProgressMonitor monitor)
			{
				try
				{
					...
					IToolsLocator[] locators = instantiateToolsLocators(
							elements); 
					...
					elements = findConfigElements(
							"com.android.ide.eclipse.ddms.traceviewLauncher"); //$NON-NLS-1$
					mTraceviewLaunchers = instantiateTraceviewLauncher(
							elements);
	 ...
	 *         found.
	 */
	private IToolsLocator[] instantiateToolsLocators(
			IConfigurationElement[] configElements) throws CoreException
	{
		...
		if (configElements.length > 0)
		{
			...
			{  
				...
				//必须是com.android.ide.eclipse.adt.ToolsLocator,删除com.android.ide.eclipse.monitor.ToolsLocator
				Object obj = configElements[i].createExecutableExtension("class"); //$NON-NLS-1$
				Logger.d(i+" ,"+obj.getClass());
	 ...
	 */
	private List<IClientAction> instantiateClientSpecificActions(
			IConfigurationElement[] elements) throws CoreException
	{
	 ...
	 * file.
	 */
	private static boolean setToolsLocation(String adbLocation,
	 ...
	 */
	@Override
	public synchronized void selectionChanged(IDevice selectedDevice,
	...
	public synchronized List<IClientAction> getClientSpecificActions()
	{
		if (mClientSpecificActions == null)
		{
			...
			IConfigurationElement[] elements = findConfigElements(
					"com.android.ide.eclipse.ddms.clientAction"); //$NON-NLS-1$
			try
			{
				mClientSpecificActions = instantiateClientSpecificActions(
						elements);
			} catch (CoreException e)
			{
